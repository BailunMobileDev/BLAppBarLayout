...
public class BLAppBarLayout extends LinearLayout implements CoordinatorLayout.AttachedBehavior {
    ...
    /**
     * Interface definition for a callback to be invoked when an {@link BLAppBarLayout}'s vertical
     * offset changes.
     */
    // TODO(b/76413401): remove this base interface after the widget migration
    public interface BaseOnOffsetChangedListener<T extends BLAppBarLayout> {

        /**
         * Called when the {@link BLAppBarLayout}'s layout offset has been changed. This allows child
         * views to implement custom behavior based on the offset (for instance pinning a view at a
         * certain y value).
         *
         * @param appBarLayout   the {@link BLAppBarLayout} which offset has changed
         * @param verticalOffset the vertical offset for the parent {@link BLAppBarLayout}, in px
         */
    ...
    public interface OnOffsetChangedListener extends BaseOnOffsetChangedListener<BLAppBarLayout> {
        void onOffsetChanged(BLAppBarLayout BLAppBarLayout, int verticalOffset);
    }
    ...
    public BLAppBarLayout(@NonNull Context context) {
        this(context, null);
    }

    public BLAppBarLayout(@NonNull Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, R.attr.appBarLayoutStyle);
    }

    public BLAppBarLayout(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    /**
     * Add a listener that will be called when the offset of this {@link BLAppBarLayout} changes.
     *
    ...
    @Override
    @NonNull
    public CoordinatorLayout.Behavior<BLAppBarLayout> getBehavior() {
        return new BLAppBarLayout.Behavior();
    }
    ...
    /**
     * Sets whether this {@link BLAppBarLayout} is expanded or not, animating if it has already been
     * laid out.
     *
     * <p>As with {@link BLAppBarLayout}'s scrolling, this method relies on this layout being a direct
    ...
    /**
     * Sets whether this {@link BLAppBarLayout} is expanded or not.
     *
     * <p>As with {@link BLAppBarLayout}'s scrolling, this method relies on this layout being a direct
     * child of a {@link CoordinatorLayout}.
     *
    ...
    /**
     * Sets whether the {@link BLAppBarLayout} is liftable or not.
     *
    ...
    /**
     * Sets whether the {@link BLAppBarLayout} is in a lifted state or not.
     *
    ...
    /**
     * Sets whether the {@link BLAppBarLayout} lifts on scroll or not.
     *
     * <p>If set to true, the {@link BLAppBarLayout} will animate to the lifted, or elevated, state when
    ...
    /**
     * Returns whether the {@link BLAppBarLayout} lifts on scroll or not.
     */
    ...
    /**
     * Sets the id of the view that the {@link BLAppBarLayout} should use to determine whether it should
     * be lifted.
     */
    ...
    /**
     * Returns the id of the view that the {@link BLAppBarLayout} should use to determine whether it
     * should be lifted.
     */
    @IdRes
    ...
    /**
     * A {@link ViewGroup.LayoutParams} implementation for {@link BLAppBarLayout}.
     */
    ...
    /**
     * The default {@link Behavior} for {@link BLAppBarLayout}. Implements the necessary nested scroll
     * handling with offsetting.
     */
    // TODO(b/76413401): remove the base class and generic type after the widget migration is done
    public static class Behavior extends BaseBehavior<BLAppBarLayout> {

        /**
         * Callback to allow control over any {@link BLAppBarLayout} dragging.
         */
        public abstract static class DragCallback extends BaseBehavior.BaseDragCallback<BLAppBarLayout> {
        }
    ...
    protected static class BaseBehavior<T extends BLAppBarLayout> extends HeaderBehavior<T> {
        ...
        public abstract static class BaseDragCallback<T extends BLAppBarLayout> {
            /**
             * Allows control over whether the given {@link BLAppBarLayout} can be dragged or not.
             *
        ...
        public void onNestedScroll(
            ...
            if (dyUnconsumed < 0) {
                ...
                consumed[1] =
                        scroll(coordinatorLayout, child, dyUnconsumed, -child.getDownNestedScrollRange(), reboundLength);
            }
        }
...
//                snapToChildIfNeeded(coordinatorLayout, abl);
                snapHeadIfNeed(coordinatorLayout, abl);
        ...
        private void snapHeadIfNeed(CoordinatorLayout coordinatorLayout, T view) {
            int offset = getTopBottomOffsetForScrollingSibling();
            if (offset > 0) {
                animateOffsetTo(coordinatorLayout, view, 0, 0);
            }

        }

        /**
         * Set a callback to control any {@link BLAppBarLayout} dragging.
         *
...
//            snapToChildIfNeeded(parent, layout);
        ...
        private int interpolateOffset(@NonNull T layout, final int offset) {
            ...
            for (int i = 0, z = layout.getChildCount(); i < z; i++) {
                ...
                final BLAppBarLayout.LayoutParams childLp = (LayoutParams) child.getLayoutParams();
                final Interpolator interpolator = childLp.getScrollInterpolator();
        ...
        private static View getAppBarChildOnOffset(
                @NonNull final BLAppBarLayout layout, final int offset) {
            final int absOffset = Math.abs(offset);
            for (int i = 0, z = layout.getChildCount(); i < z; i++) {
                final View child = layout.getChildAt(i);
                if (absOffset >= child.getTop() && absOffset <= child.getBottom()) {
        ...
        /**
         * A {@link Parcelable} implementation for {@link BLAppBarLayout}.
         */
     ...
     * scrolling to automatically scroll any {@link BLAppBarLayout} siblings.
     */
    public static class ScrollingViewBehavior extends HeaderScrollingViewBehavior {
        ...
        public boolean onRequestChildRectangleOnScreen(
                ...
                boolean immediate) {
            final BLAppBarLayout header = findFirstDependency(parent.getDependencies(child));
            if (header != null) {
        ...
        float getOverlapRatioForOffset(final View header) {
            if (header instanceof BLAppBarLayout) {
                final BLAppBarLayout abl = (BLAppBarLayout) header;
        ...
        private static int getAppBarLayoutOffset(@NonNull BLAppBarLayout abl) {
            final CoordinatorLayout.Behavior behavior =
        ...
        @Nullable
        @Override
        BLAppBarLayout findFirstDependency(@NonNull List<View> views) {
        ...
        int getScrollRange(View v) {
            if (v instanceof BLAppBarLayout) {
                return ((BLAppBarLayout) v).getTotalScrollRange();
            } else {
        ...
        private void updateLiftedStateIfNeeded(View child, View dependency) {
            if (dependency instanceof BLAppBarLayout) {
                BLAppBarLayout BLAppBarLayout = (BLAppBarLayout) dependency;
                if (BLAppBarLayout.isLiftOnScroll()) {
                    BLAppBarLayout.setLiftedState(BLAppBarLayout.shouldLift(child));
                }
            }
        }
    }
